...
# Plan-level machinery lives with the plan format in Gaunt_utils: every one
# of these knows the internal array names (term_L, term_off, cell_start,
# term_gaunt_bare), so a future change of format is a one-file edit.
...
drho  = rho_m - rho_c
drhol = rho_c - rho_l
T_c   = 75.0             # Arbitrary crustal thickness value, TBC
Te_input = 268.12e3
...
# lmax settings
LMAX_RUNS  = [90]        # last entry is the reference resolution
LMAX_REF = max(LMAX_RUNS)
grid_expansion_res = LMAX_REF * 3

rotate_angles = (15.0, 43.0, 155.0)
# rotate_angles = (0.0, 0.0, 0.0)
lmax_Te_fit = LMAX_RUNS[-1]

# Set which Te map is used, strain-14, strain-17, or strain-0, or 'twoval'
# (0 returns constant Te map with Te=Te_input)
# ('twoval' returns custom two-value Te map with Tharsis as one of the values)
strain = 14
...
Te_twoval_big = 130.0e3
Te_twoval_sml = 70.0e3
...
# %% CORRECTION SELF-TESTS
 
# Five checks on the correction functions, each against a case whose answer is
# known in CLOSED FORM. They never run in production -- removing them cannot
# change a result -- but they catch the class of error an end-to-end run
# cannot: one that is small in the converged solution yet wrong in the
# operator. selftest_drho_w_interface exists precisely because its absence let
# such a bug through (relief vs absolute shape at the w interface, which
# diverged the Picard loop on iteration 1).
#
# All five are driven by selftest_corrections() at the bottom of this cell.
 
def selftest_omega_corr_rhs(topo_clm, geoid_clm, T_e_parent, a_clm,
                            lmax, R, T_e_0, g0, mass, tol=1e-10):
    """
    Machine-precision check of `omega_corr_to_rhs` against the PRODUCTION
    builders -- not against a second transcription of the same algebra.
 
    METHOD. The H-terms of Omega_eq1_RHS (factor1a/1b on the Te*H fields) and
    Omega_eq2_RHS (factor2a on Delta'(alpha*Te*H)) ARE this operator applied to
    the H-part of DSP eq (5),
        omega_H = nu/(1-nu) * rho_l * g0 * Te * H / R .
    So feeding omega_H to the generic operator must reproduce them exactly.
 
    The dc and drho blocks of both builders are gated on the module-level
    `solve_for`; setting it to a value matching neither branch leaves ONLY the
    H terms, so the production functions themselves serve as the reference. The
    global is restored in a `finally`.
 
    RESOLUTION CAVEAT -- read before interpreting a failure. The test object
    routes omega through coefficients (om_clm -> grid), while Omega_eq2_RHS
    forms Te*H*alpha as a single TRIPLE grid product with no intermediate
    truncation. The two paths therefore agree only while Te*H fits inside the
    2*lmax+1 working band. Measured on synthetic fields:
 
        constant Te (strain = 0)         eq1 1.6e-16   eq2 1.6e-15
        Te structured at degrees 2-5     eq1 1.5e-16   eq2 2.2e-15
        Te with power to degree 2*lmax   eq1 1.7e-16   eq2 7.3e-03
 
    Note WHICH one moves. eq1 is exact in all three cases, because both paths
    truncate at the same point; only eq2 is exposed, and only once Te itself is
    broadband enough that Te*H overruns the band. That is a property of the
    TEST -- it feeds omega as coefficients where production never does -- not
    of the operator. Judge the operator at strain = 0; treat a variable-Te
    miss as a band-limit report, and check it against 2*lmax before suspecting
    the algebra.
    """
    global solve_for, filter_type
    _saved = solve_for
    _saved_filter = filter_type
    Re = R - T_e_0/2.0
    try:
        # Strip the dc and drho blocks from the two builders by setting
        # solve_for to a value matching neither branch, leaving ONLY the H
        # terms -- so the production functions themselves become the reference.
        solve_for = '_selftest_H_terms_only'
        # Neutralise the filter too. DCfilters() reads solve_for, and the
        # sentinel above is neither branch, so without this the builders would
        # silently be evaluated with the drhom filter pair live. The H terms
        # compared here do not use those factors, so it makes no numerical
        # difference today; it is off so the test measures the omega OPERATOR
        # alone and cannot be perturbed by a future change to the filter code.
        filter_type = None
        ref1 = Omega_eq1_RHS(topo_clm, geoid_clm, T_e_parent,
                             lmax=lmax, R=R, T_e_0=T_e_0, Re=Re, g0=g0, mass=mass)
        ref2 = Omega_eq2_RHS(topo_clm, geoid_clm, T_e_parent, a_clm,
                             lmax=lmax, R=R, T_e_0=T_e_0, Re=Re, g0=g0, mass=mass)
    finally:
        solve_for = _saved
        filter_type = _saved_filter
 
    # omega_H, built exactly as DSP eq (5) writes it: a GRID product Te*H.
    Te_grid = T_e_parent.expand(lmax=grid_expansion_res).data
    H_grid  = topo_clm.expand(lmax=grid_expansion_res).data - R
    TeH     = pysh.SHGrid.from_array(Te_grid*H_grid).expand()
    om_H    = (nu/(1.0 - nu))*rho_l*g0/R * TeH.coeffs[:, :2*lmax+1, :2*lmax+1]
 
    tst1, tst2 = omega_corr_to_rhs(om_H, T_e_parent, a_clm, lmax, R, T_e_0, Re)
 
    e1 = np.max(np.abs(tst1 - ref1))/max(np.max(np.abs(ref1)), 1e-300)
    e2 = np.max(np.abs(tst2 - ref2))/max(np.max(np.abs(ref2)), 1e-300)
    ok = (e1 < tol) and (e2 < tol)
    print(f'  \nselftest_omega_corr_rhs: rel err eq1 = {e1:.3e}, '
          f'eq2 = {e2:.3e}   -> {"PASS" if ok else "FAIL"}')
    if not ok:
        print('    \n(at variable Te a truncation-level miss is expected -- '
              'see the docstring. Re-run at strain = 0 to judge the operator.)')
    return ok
 
 
def selftest_drho_q_correction(R, lmax=12, tol=1e-12):
    """
    Two properties of `drho_q_omega_correction` that are checkable without DSP.
 
    (1) ZERO INPUT -> ZERO OUTPUT, exactly. Guarantees the term is inert
        whenever the density variations vanish, which is what makes
        `density_var = False` a valid regression gate.
 
    (2) THE PRODUCT IS SPATIAL, NOT COEFFICIENT-WISE. Feeding a MONOPOLE
        density field drho = k (a constant) must return exactly
        g0 * k * (H - G)_lm, because multiplying by a constant commutes with
        the expansion. This is the one case where the true answer is known in
        closed form, and it fails loudly for the classic error of writing
        drho_lm * HG_lm coefficient-by-coefficient -- that would return
        k*(H-G) only at degree 0 and zero everywhere else.
 
    It does NOT verify the physics against DSP; that needs the instrumented
    one-Picard-step comparison of handover 7A.3.
    """
    _GRID_CACHE.clear()   # synthetic fields: see _cached_grid
    rng = np.random.default_rng(7)
    topo = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    geoid = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    for l in range(2, lmax+1):
        for m in range(0, l+1):
            topo.coeffs[0, l, m] = rng.normal(0, 1.0e3)
            geoid.coeffs[0, l, m] = rng.normal(0, 1.0e2)
            if m > 0:
                topo.coeffs[1, l, m] = rng.normal(0, 1.0e3)
                geoid.coeffs[1, l, m] = rng.normal(0, 1.0e2)
    topo.coeffs[0, 0, 0] = R
    geoid.coeffs[0, 0, 0] = R
    g0_t = 3.71
 
    # (1) zero density -> zero correction
    z = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    _Te1 = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    _Te1.coeffs[0, 0, 0] = 1.0
    e_zero = np.abs(drho_q_omega_correction('H', z, topo, geoid, None,
                                            _Te1, R, lmax, g0_t)[0]).max()
 
    # (2) monopole density -> exact closed form
    k = 137.0
    mono = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    mono.coeffs[0, 0, 0] = k
    got = drho_q_omega_correction('H', mono, topo, geoid, None,
                                  _Te1, R, lmax, g0_t)[0]
    HG = topo.coeffs - geoid.coeffs
    HG[0, 0, 0] = 0.0
    want = g0_t * k * HG
    e_mono = np.abs(got - want).max()/max(np.abs(want).max(), 1e-300)
 
    ok = (e_zero == 0.0) and (e_mono < tol)
    print(f'  selftest_drho_q_correction: zero-in = {e_zero:.3e}, '
          f'monopole rel err = {e_mono:.3e}   -> {"PASS" if ok else "FAIL"}')
    return ok
 
 
def selftest_drho_omega_correction(R, g0, Te0, lmax=12, tol=1e-12):
    """
    Same two properties as the q self-test, plus the one that matters most
    here: that the ABSOLUTE-shape term is present.
 
    (1) zero density -> exactly zero.
    (2) monopole drho = k, constant Te -> closed form
            v1v * g0 * Te0/R * k * H_lm,   H_lm[0,0,0] = R,
        which simultaneously checks the scalar-Te reduction and that H carries
        R. An implementation that used the relief instead of the shape passes
        every degree >= 1 of this test and fails only at degree 0 -- so the
        test compares degree 0 explicitly.
    """
    _GRID_CACHE.clear()   # synthetic fields: see _cached_grid
    ...
    for l in range(2, lmax+1):
        for m in range(0, l+1):
            ...
            if m > 0:
                topo.coeffs[1, l, m] = rng.normal(0, 1.0e3)
    topo.coeffs[0, 0, 0] = R
    Te = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    Te.coeffs[0, 0, 0] = Te0
 
    z = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    e_zero = np.abs(drho_q_omega_correction('H', z, topo, topo, None,
                                            Te, R, lmax, g0)[1]).max()
 
    k = 91.0
    mono = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    mono.coeffs[0, 0, 0] = k
    got  = drho_q_omega_correction('H', mono, topo, topo, None,
                                   Te, R, lmax, g0)[1]

    H_lm = topo.coeffs.copy()
    H_lm[0, 0, 0] = 0.0
    want = (nu/(1.0 - nu))*g0*Te0/R * k * H_lm
    e_mono = np.abs(got - want).max()/max(np.abs(want).max(), 1e-300)
    # Degree 0 is the DISCRIMINATING coefficient: an implementation using the
    # wrong H passes every degree >= 1 and fails only here. With 'relief' the
    # correct answer at l = 0 is ZERO, so compare against the field's scale
    # rather than against want[0,0,0], which would be a division by zero.
    e_deg0 = abs(got[0, 0, 0] - want[0, 0, 0])/max(np.abs(want).max(), 1e-300)
 
    ok = (e_zero == 0.0) and (e_mono < tol) and (e_deg0 < tol)
    print(f'  selftest_drho_omega_correction: '
          f'zero-in = {e_zero:.3e}, monopole rel err = {e_mono:.3e}, '
          f'deg-0 = {e_deg0:.3e}   -> {"PASS" if ok else "FAIL"}')
    return ok
 
 
def selftest_drho_geoid_correction(R, mass, lmax=12, lmaxgrid=120, tol=1e-9):
    """
    The reduction that makes the density branch consistent with the pure-FA one.
 
    With rho_ref = rho_l and drho = 0 the density grid is uniformly rho_l, so
    `(rho_grid - rho_l) = 0` everywhere and the correction must VANISH at
    nmax = 1 -- the density-branch analogue of the "nmax = 1, density_var =
    False -> exactly zero" free regression test of handover section 2, which
    does NOT transfer here because DSP forces density_var on in this branch.
 
    A nonzero result would mean the mass-sheet subtraction and CilmPlusRhoHDH
    disagree for l >= 1, i.e. the two sides are not being referenced alike.
 
    THE SCALE MATTERS. The cancellation happens at rho_l = 3200 while the signal
    of interest is the drho field, so the residual has a floor of roughly
    rho_l * eps * (amplification) ~ 1e-11 in absolute terms. Normalising by a
    1 kg/m3 test signal would report that floor as ~1.6e-11 and look like a
    failure; normalising by a REPRESENTATIVE drho -- the measured field has a
    spatial rms near 200 kg/m3 -- puts it at ~1e-13, which is what it is.
    """
    _GRID_CACHE.clear()   # synthetic fields: see _cached_grid
    ...
    for l in range(2, lmax+1):
        for m in range(0, l+1):
            topo.coeffs[0, l, m] = rng.normal(0, 2.0e3)
            if m > 0:
                topo.coeffs[1, l, m] = rng.normal(0, 2.0e3)
    topo.coeffs[0, 0, 0] = R
    zero_drho = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
 
    H_corr = interface_geoid_correction('H', R, mass, lmax, lmaxgrid, 1,
                                       topo_clm=topo, drho_clm=zero_drho)
    # scale: the correction a REPRESENTATIVE density variation produces
    ref_amp = 200.0
    ref = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    ref.coeffs[0, 0, 0] = ref_amp
    dH_ref = interface_geoid_correction('H', R, mass, lmax, lmaxgrid, 1,
                                       topo_clm=topo, drho_clm=ref)
    scale = max(np.abs(dH_ref).max(), 1e-300)
    err = np.abs(H_corr).max()/scale
    ok = err < tol
    print(f'  selftest_drho_geoid_correction: uniform-rho_l residual = '
          f'{err:.3e} (of a {ref_amp:.0f} kg/m3 signal)'
          f'   -> {"PASS" if ok else "FAIL"}')
    return ok
 
 
def selftest_drho_w_interface(R, g0, Te0, lmax=12, tol=1e-12):
    """
    The RELIEF-vs-ABSOLUTE-SHAPE check for the 'w' interface.
 
    The existing q/omega self-tests only ever exercised 'H'. That gap let a
    real error through: `drho_q_omega_correction('w', ...)` was written with
    w[0,0,0] = R (absolute shape), copying the H interface, when DSP's line
    1872 uses the relief. The correction came out R/|w| ~ 68x too large and
    the Picard loop diverged on the first iteration.
 
    A monopole drho = k gives, in closed form,
        q_corr  = -g0 * k * w_relief_lm
        om_corr = -v1v * g0 * Te0/R * k * w_relief_lm
    with w_relief_lm[0,0,0] = 0 -- note the MINUS (the w gate forces
    top_drho == 0, so DSP takes the -rho_grid_var branch) and note that
    DEGREE 0 OF THE ANSWER IS ZERO. That last point is the whole test: an
    absolute-shape implementation puts g0*k*R there instead, which is 68x the
    largest l >= 1 coefficient and impossible to miss.
    """
    _GRID_CACHE.clear()   # synthetic fields: see _cached_grid
    rng = np.random.default_rng(31)
    w = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    for l in range(2, lmax+1):
        for m in range(0, l+1):
            w.coeffs[0, l, m] = rng.normal(0, 5.0e4)
            if m > 0:
                w.coeffs[1, l, m] = rng.normal(0, 5.0e4)
    w.coeffs[0, 0, 0] = 0.0
    Te = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    Te.coeffs[0, 0, 0] = Te0
    topo = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    ...
    mono = pysh.SHCoeffs.from_zeros(lmax=lmax, normalization='4pi')
    mono.coeffs[0, 0, 0] = k
    q_got, om_got = drho_q_omega_correction('w', mono, topo, topo, w,
                                            Te, R, lmax, g0)
    q_want  = -g0 * k * w.coeffs
    om_want = -(nu/(1.0 - nu)) * g0 * Te0/R * k * w.coeffs
 
    e_q  = np.abs(q_got - q_want).max()/max(np.abs(q_want).max(), 1e-300)
    e_om = np.abs(om_got - om_want).max()/max(np.abs(om_want).max(), 1e-300)
    # degree 0 is the discriminating coefficient
    e_d0 = abs(q_got[0, 0, 0])/max(np.abs(q_want).max(), 1e-300)
 
    ok = (e_q < tol) and (e_om < tol) and (e_d0 < tol)
    print(f'  selftest_drho_w_interface: q rel err = {e_q:.3e}, '
          f'omega rel err = {e_om:.3e}, deg-0 leakage = {e_d0:.3e}'
          f'   -> {"PASS" if ok else "FAIL"}\n')
    return ok
 
 
def selftest_corrections(R, mass, g0, T_e_0, topo_p=None, geoid_p=None,
                         T_e_parent=None, a_clm=None, LMAX_REF=None):
    """
    Run all five and return True only if every one passes.
 
    The omega-hook test needs the real input fields; the other four are
    self-contained on synthetic ones.
 
    READ BEFORE TREATING AN OMEGA FAILURE AS REAL. selftest_omega_corr_rhs
    feeds omega as COEFFICIENTS, which production never does, so its eq-2
    number degrades once Te is broadband enough that Te*H overruns the
    2*lmax+1 band. At strain = 0 expect ~1e-16 (eq 1) and ~1e-14 (eq 2); at
    variable Te a larger eq-2 number is a property of the TEST, not of the
    operator, because eq 1 stays exact either way -- both paths truncate at
    the same point. So: eq1 large -> real problem. eq2 large with eq1 fine at
    variable Te -> the test, not the code.
    """
    ok = []
    if None in (topo_p, geoid_p, T_e_parent, a_clm, LMAX_REF):
        print('  selftest_omega_corr_rhs: SKIPPED (input fields not given)\n')
    else:
        ok.append(selftest_omega_corr_rhs(
            truncate(topo_p, LMAX_REF), truncate(geoid_p, LMAX_REF),
            T_e_parent, a_clm, LMAX_REF, R, T_e_0, g0, mass))
    ok.append(selftest_drho_q_correction(R))
    ok.append(selftest_drho_omega_correction(R, g0, T_e_0))
    ok.append(selftest_drho_geoid_correction(R, mass))
    ok.append(selftest_drho_w_interface(R, g0, T_e_0))
    if not all(ok):
        print('  !! ONE OR MORE CORRECTION SELF-TESTS FAILED -- read the note '
              'in selftest_corrections before assuming the model is wrong.\n')
    return all(ok)
...
if __name__ == "__main__":
    ...
    # Run selftest on all the newly added corrections
    selftest_corrections(R, mass, g0, T_e_0,
                         topo_p=topo_p, geoid_p=geoid_p,
                         T_e_parent=T_e_parent, a_clm=a_clm,
                         LMAX_REF=LMAX_REF)
...
# %% PLOTS - 2D plot of gaunt cell i vs cell j (output mode vs input mode)
        
    if 'M' in cache:
        M_op     = cache['M']
        N_op     = cache['N']
        mode_map_op = cache['mode_map']
        l_of_mode   = np.array([l for (l, m) in mode_map_op])
     
        # The w-w block (top-left part of matrix M).
        # This is where the D-field (and hence Te) coupling lives most directly
        # The full 2N x 2N picture is the same story four times
        A_blk = M_op[:N_op, :N_op]
     
        # Degrees 0 and 1 are pinned in the assembly (M[idx,:] = 0, M[idx,idx] = 1),
        # so also exclude them from every statistic below
        keep    = l_of_mode >= 2
        A_keep  = A_blk[np.ix_(keep, keep)]
        l_keep  = l_of_mode[keep]
     
        # ---- panel 1: magnitude map ------------------------------------------
        # Log scale, normalised to the largest entry, clipped to 12 decades. A
        # linear scale shows nothing: the diagonal is orders above everything else.
        A_norm  = np.abs(A_keep) / np.abs(A_keep).max()
        floor   = 1e-12
        A_show  = np.log10(np.maximum(A_norm, floor))
     
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6.2))
        im = ax1.imshow(A_show, cmap='magma', vmin=np.log10(floor), vmax=0.0,
                        interpolation='nearest')
        plt.colorbar(im, ax=ax1, label=r'$\log_{10}\,|A_{ij}| / \max|A|$')
        deg_edge = np.searchsorted(l_keep, np.arange(2, LMAX_REF + 1))
        for b in deg_edge[::5]:
            ax1.axhline(b, lw=0.3, color='cyan', alpha=0.4)
            ax1.axvline(b, lw=0.3, color='cyan', alpha=0.4)
        ax1.set_title(f'Assembled w-w operator, lmax={LMAX_REF}, strain={strain}')
        ax1.set_xlabel('input mode index (l >= 2)')
        ax1.set_ylabel('output mode index (l >= 2)')
     
        # ---- panel 2: coupling reach -----------------------------------------
        # THE QUANTITATIVE ANSWER. For each degree separation dl = |l - l'|, the
        # RMS of the operator entries at that separation, normalised to dl = 0.
        # Constant Te  -> a spike at dl = 0 and roundoff (~1e-15) elsewhere.
        # Variable Te  -> a decaying tail whose width IS the coupling reach, set
        #                 by the degree content of Te through D and alpha.
        dl_grid = np.abs(l_keep[:, None] - l_keep[None, :])
        dl_max  = int(dl_grid.max())
        reach   = np.full(dl_max + 1, np.nan)
        for d in range(dl_max + 1):
            sel = (dl_grid == d)
            if sel.any():
                reach[d] = np.sqrt(np.mean(A_keep[sel]**2))
        reach /= reach[0]
     
        ax2.semilogy(np.arange(dl_max + 1), reach, '.-', lw=1.2, ms=4)
        ax2.axhline(1e-15, color='grey', ls=':', lw=1,
                    label='roundoff floor (~1e-15)')
        ax2.set_xlabel(r"degree separation  $|l - l'|$")
        ax2.set_ylabel(r'RMS$|A|$ at that separation, / RMS at $\Delta l=0$')
        ax2.set_title('Coupling reach: how far Te spreads the operator')
        ax2.grid(True, which='both', alpha=0.3)
        ax2.legend()
     
        fig.suptitle('The PLAN is the same for any Te; the VALUES are not.\n'
                     'Constant Te collapses this to the diagonal, which is what '
                     'lets DSP solve 5x5 per degree.', fontsize=10)
        plt.tight_layout()
            
        if SaveFigs:
            plt_savetitle = (f'GauntCoupling_grid_lmax={LMAX_REF}_iter{iterate}_nmax={nmax}_delta_max={delta_max}_'
        ...
        # Fraction of the operator's Frobenius energy that sits OFF the
        # degree-diagonal. Zero (to roundoff) at constant Te; a real number at
        # variable Te, and it is a direct measure of how much the 2N x 2N
        # formulation is actually buying you over DSP's per-degree solve.
        diag_mask = (dl_grid == 0)
        off_frac  = (np.sum(A_keep[~diag_mask]**2)
                     / np.sum(A_keep**2))
        print(f'\n  strain = {strain}, lmax = {LMAX_REF}')
        print(f'    off-degree-diagonal energy fraction : {off_frac:.6e}')
        print(f'    coupling reach at 1e-6 of the peak  : dl = '
              f'{int(np.argmax(reach < 1e-6)) if np.any(reach < 1e-6) else dl_max}')
        print('    reach at dl = 1, 2, 5, 10           : '
              + ', '.join(f'{reach[d]:.2e}' for d in (1, 2, 5, 10) if d <= dl_max))
     
        # ---- save the reach curve so two runs can be overlaid ----------------
        # Same pattern as the grid self-comparison: run once at strain = 0, once
        # at strain = 14, and the second run plots both.
        _reach_dir = 'M5_coupling_reach'
        os.makedirs(_reach_dir, exist_ok=True)
        _reach_f = f'{_reach_dir}/reach_l{LMAX_REF}_st{strain}_Te{Te_input/1e3}.npy'
        np.save(_reach_f, reach)
        _others = sorted(f for f in os.listdir(_reach_dir)
                         if f.startswith(f'reach_l{LMAX_REF}_')
                         and f != os.path.basename(_reach_f))
        if _others:
            plt.figure(figsize=(7.5, 5))
            plt.semilogy(reach, '.-', lw=1.4, label=f'strain = {strain}')
            for _o in _others:
                _r = np.load(f'{_reach_dir}/{_o}')
                _st = _o.split('_st')[1].split('_')[0]
                plt.semilogy(_r, '.--', lw=1.2, label=f'strain = {_st}')
            plt.axhline(1e-15, color='grey', ls=':', lw=1)
            plt.xlabel(r"degree separation  $|l - l'|$")
            plt.ylabel('normalised RMS coupling')
            plt.title('Coupling reach: constant vs variable Te')
            plt.grid(True, which='both', alpha=0.3)
            plt.legend()
            plt.tight_layout()
            
            if SaveFigs:
                plt_savetitle = (f'GauntCoupling_degrees_lmax={LMAX_REF}_iter{iterate}_nmax={nmax}_delta_max={delta_max}_'
                        + f'SolveFor{solve_for}_'
                        + (f'Mb{Mb/1e3}_Mt{Mt/1e3}_' if solve_for == 'drho_lm' else '')
                        + (f'both_constant_Te={Te_input/1e3}km_' if strain==0 else '')
                        + (f'Te_M5=PlesaStrain14Map_'
        ...
        print("  cache['M'] not present -- add the cache write from snippet 1 and "
              "rerun the solve.")
...
# %% PLOTS - POWER SPECTRUM w + RESIDUAL RATIO IF ROTATION IS APPLIED
    ...
    if do_rotation_check:
        ...
        ax2.grid(True)
        ax2.set_title('M5 - Ratio unrotated over rotated power spectra of w')
    else:
        ax2.set_visible(False)

    ax1.set_title('M5 - Power spectra of w ' 
                  + ('(Plesa Te Map)' if strain != 0 else '' ))
    ax1.legend()
    ax1.set_ylim(1e-2)
    plt.tight_layout()
    if SaveFigs:
        plt1_title = (f'M5 - Power spectra w, lmax_run={LMAX_RUNS}, '
...
# %% PLOTS - POWER SPECTRUM OF RESIDUAL BETWEEN LAST TWO LMAX RUNS
    ...
    if len(LMAX_RUNS)>1:
        ...
        ax2.legend(); 
        ax2.grid(True)
        ax2.set_title(f'M5 - Residual vs lmax_ref={LMAX_REF}')
        plt.tight_layout(); 
        if SaveFigs:
            plt2_title = (f'M5 - Residuals w power, lmax_run={LMAX_RUNS}, '
                          f'lmaxTe={lmax_Te_fit}'
                          + (f', rotated {rotate_angles}' if rotation else '') 
                          + '.png')
...
# %% PLOTS - 2D DEFLECTION MAP + RESIDUAL BETWEEN LAST TWO LMAX RUNS
    ...
    if len(LMAX_RUNS)>1:
        ...
        if len(LMAX_RUNS)>1:
            ...
            a1.set_title(f'M5 - Transverse displacement w Beuthe-model (lmax={LMAX_REF})'
                         + (f', rot={rotate_angles}' if rotation else ''))
            ...
            a2.set_title(f'M5 - Residual w: lmax={LMAX_REF} minus lmax={lo}'
                         + (f', rot={rotate_angles}' if rotation else ''))
        ...
        plt.tight_layout()
        if SaveFigs:
            plt4_title = (f'M5 - Displacement w 2D map, lmax_run={LMAX_RUNS}, '
                          f'lmaxTe={lmax_Te_fit}'
                          + (f', rotated {rotate_angles}' if rotation else '') 
                          + '.png')
...
# %% PLOTS - DSP-M5 RESIDUAL PLOTS
    ...
    plt.suptitle('Residual checks DSP and M5 - '
                 ...
                 + f'DSP & M5 constant $T_c$={T_c/1e3} km\n'
                 + (f'Mb={Mb/1e3} km, Mt={Mt/1e3} km, ' if solve_for == 'drho_lm' else '')
...
# %% PLOTS - w-POWER SPECTRA COMPARISONS BETWEEN DSP AND M5
    
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10,9))

    solutions_w[lmax_run, rotation].plot_spectrum(ax=ax1, show=False, 
                    legend=('M5 w-coeffs'))
    w_coeffs_DSP_copy = w_coeffs_DSP.copy()
    w_coeffs_DSP_copy.coeffs[0,0,0] = 0
    w_coeffs_DSP_copy.plot_spectrum(ax=ax1, show=False, 
                    legend=('DSP coeffs'), plot_dict={'linestyle': '--'})
    ax1.set_title('M5 & DSP w-coeffs')

    w_spectrum_diff = (w_coeffs_DSP- solutions_w[LMAX_REF, 0]).spectrum() 
    l = np.arange(0,(LMAX_REF+1))
    ax2.plot(l, w_spectrum_diff, 
             label=('DSP - M5'))
    ax2.set_title('|DSP-M5| w-residual')
    plt.tight_layout()
    plt.grid()
    ax1.set_ylim(1e-2)
    ax2.set_xlim(0,44)
    if SaveFigs:
        plt_savetitle = (f'Comparison_w_DSP_M5_lmax={LMAX_REF}_iter{iterate}_nmax={nmax}_delta_max={delta_max}_'
                + f'SolveFor{solve_for}_'
                + (f'Mb{Mb/1e3}_Mt{Mt/1e3}_' if solve_for == 'drho_lm' else '')
                + (f'both_constant_Te={Te_input/1e3}km_' if strain==0 else '')
                + (f'Te_M5=PlesaStrain14Map_'
...
# %% PLOTS - w-POWER SPECTRA 2D COMPARISONS BETWEEN DSP AND M5
    ...
    ax1.set_title('M5 w-coeffs')
    
    w_coeffs_DSP_copy.plot_spectrum2d(ax=ax2, show=False, cmap_limits=[1e-6,1e6])
...
# %% DSP A_lm vs M5 S_lm
    ...
    Omega_lm_M5 = compute_Omega(solutions_w[LMAX_REF,0], T_e_parent, topo_clm,
                                solutions_g_ms[LMAX_REF,0], solutions_q[LMAX_REF,0],
                                g0, R=R, T_e_0=T_e_0, lmax_calc=LMAX_REF,
                                lmax_grid=grid_expansion_res,
                                omega_corr_phys=solutions_omega_corr.get((LMAX_REF, 0)))
    Omega_lm_M5_DSPconv = Omega_lm_M5.copy()
    Omega_lm_M5_DSPconv.coeffs = Omega_lm_M5_DSPconv.coeffs/(R-T_e_0/2)
    Omega_grid_M5 = pysh.SHCoeffs.from_array(Omega_lm_M5_DSPconv.coeffs).expand(**args_expand)


    # NOTE: this was `Omega_lm_DSP = A_lm = ...`, binding BOTH names to the
    # same object (handover section 4). A_lm is rebound below so it was
    # harmless, but the pattern is the one that has bitten before.                            
    Omega_grid_DSP = pysh.SHCoeffs.from_array(Omega_lm_DSP.coeffs).expand(**args_expand)

    Omega_diff_DSPM5 = Omega_grid_DSP.copy()
    Omega_diff_DSPM5.data = Omega_grid_DSP.data - Omega_grid_M5.data


    S_lm = cons_disp_S(solutions_w[LMAX_REF,0], solutions_F[LMAX_REF,0], Omega_lm_M5, T_e_parent, a_clm, R=R, T_e_0=T_e_0, lmax_calc=LMAX_REF, lmax_grid=grid_expansion_res)
    ...
    A_S_diff_DSPM5 = A_grid.copy()
    A_S_diff_DSPM5.data = A_grid.data - S_grid.data
    ...
    cmap_limits_Omega = [Omega_grid_M5.data.min(), Omega_grid_M5.data.max()]
    
    Omega_min, Omega_max = Omega_diff_DSPM5.data.min(), Omega_diff_DSPM5.data.max()
    ...
    Omega_grid_M5.plot(ax=ax2, 
                cmap_limits=cmap_limits_Omega, 
                cmap=cmap3, colorbar=None, ticks='wsen', xlabel=None, ylabel=None, **args_plot)
    ax2.set_title('M5 - Consoidal load potential Omega_lm', fontweight="bold")
    

    Omega_diff_DSPM5.plot(ax=ax3, cmap=cmap2,
                      cmap_limits = cmap_limits_Omega_diff,
                      colorbar=None, ticks='wsen', xlabel=None, ylabel=None, **args_plot)
    ax3.set_title('Consoidal load potential residual Omega DSP - M5', fontweight="bold")
    ...
    ax2.contour(Omega_grid_M5.data>0, levels=[0.99], extent=(0,360,-90,90), colors='k', origin='upper')
    ...
    A_S_min, A_S_max = A_S_diff_DSPM5.data.min(), A_S_diff_DSPM5.data.max()
    ...
    S_grid.plot(ax=ax5, 
                cmap_limits=cmap_limits_S, 
                cmap=cmap3, colorbar=None, ticks='wsen', xlabel=None, ylabel=None, **args_plot)
    ax5.set_title('M5 - Consoidal displacement S_lm', fontweight="bold")
    

    A_S_diff_DSPM5.plot(ax=ax6, cmap=cmap2,
                      cmap_limits = cmap_limits_A_S_diff,
                      colorbar=None, ticks='wsen', xlabel=None, ylabel=None, **args_plot)
    ax6.set_title('Consoidal displacement residual A-S DSP - M5', fontweight="bold")
    ...
    # --- GLOBAL SUPTITLE AND OUTPUT ---
    plt.suptitle('Residual A_lm DSP and S_lm M5 - '
                 + (f'{filter_type} filtered at l={filter_half}\n' if filter_on else 'unfiltered fin amp\n')
                 + ('Solving for $\\delta \\rho_{lm}$, $\\delta c_{lm}$=0' if solve_for == 'drho_lm' else '')
                 + ('Solving for $\\delta c_{lm}$, $\\delta \\rho_{lm}$=0' if solve_for == 'dc_lm' else '')
                 + f' --- lmax={LMAX_REF}, nmax={nmax}, delta_max={delta_max}'
    ...
    if SaveFigs:
        plt_savetitle = (f'Residuals_Omega_AS_DSP_M5_lmax={LMAX_REF}_iter{iterate}_nmax={nmax}_delta_max={delta_max}_'
                + f'SolveFor{solve_for}_'
                + (f'Mb{Mb/1e3}_Mt{Mt/1e3}_' if solve_for == 'drho_lm' else '')
                + (f'both_constant_Te={Te_input/1e3}km_' if strain==0 else '')
                + (f'Te_M5=PlesaStrain14Map_'
...
# %% PLOTS - RESIDUALS OMEGA AND A/S DSP-M5
    ...
    Omega_lm_M5_DSPconv.plot_spectrum(ax=ax1, show=False, 
                    legend=('M5 Omega coeffs'))
    Omega_lm_DSP.plot_spectrum(ax=ax1, show=False, 
                    legend=('DSP Omega coeffs'), plot_dict={'linestyle': '--'})
    ax1.set_title('M5 & DSP Omega-coeffs')
    ax1.set_xlim(0,xlim)    
    ax1.set_ylim(1e5)    

    Omega_spectrum_diff = (Omega_lm_DSP - Omega_lm_M5_DSPconv).spectrum() 
    l = np.arange(0,(LMAX_REF+1))
    ax2.plot(l, Omega_spectrum_diff, 
             label=('DSP - M5'))
    ax2.set_title('|DSP-M5| Omega residual')
    ax2.set_xlim(0,xlim)    
    ax2.grid(True)

    S_lm.plot_spectrum(ax=ax3, show=False, 
                    legend=('M5 S_lm coeffs'))
    A_lm.plot_spectrum(ax=ax3, show=False, 
                    legend=('DSP A_lm coeffs'), plot_dict={'linestyle': '--'})
    ax3.set_title('M5 & DSP S-A -coeffs')
    ax3.set_xlim(0,xlim)    
    ax3.set_ylim(1e-5)    
    ...
    ax4.plot(l, A_S_spectrum_diff, 
             label=('DSP - M5'))
    ax4.set_title('|DSP-M5| A-S residual')
    ax4.set_xlim(0,xlim)    
...
# %% THARSIS STRAIN PLOT
    ...
    sub_res            = sub_DSP - sub_M5

    # --- colour limits: shared symmetric for the two fields, own scale for the residual
    # strain_min, strain_max = min(sub_DSP.min(), sub_M5.min()), max(sub_DSP.max(), sub_M5.max())
    strain_min, strain_max = sub_DSP.min(), sub_DSP.max()
    cmap_limits_strain = [-max(abs(strain_min), abs(strain_max)),
                           max(abs(strain_min), abs(strain_max))]
    res_min, res_max = sub_res.min(), sub_res.max()
    cmap_limits_strain_diff = [-max(abs(res_min), abs(res_max)),
                                max(abs(res_min), abs(res_max))]
    # cmap_limits_strain_diff = [-23.7, 23.7]

    # --- layout: 3 panels on top, shared colorbar under cols 0-1, residual bar under col 2
    fig = plt.figure(figsize=(16, 6))
    gs  = gridspec.GridSpec(2, 6, height_ratios=[1, 0.03], hspace=0.28, wspace=0.15)
    ax1 = fig.add_subplot(gs[0, 0:2])
    ax2 = fig.add_subplot(gs[0, 2:4])
    ax3 = fig.add_subplot(gs[0, 4:6])
    cax_strain_shared = fig.add_subplot(gs[1, 1:3])
    cax_strain_diff   = fig.add_subplot(gs[1, 4:6])

    def _decorate(ax, title, ylabel=True, contour=True):
        ax.set_xticks(np.arange(180, 331, 30)); ax.set_yticks(np.arange(-75, 76, 25))
        ax.grid(True); ax.set_xlabel('Longitude')
        if ylabel: ax.set_ylabel('Latitude')
        else:      ax.set_yticklabels([])
        ax.set_title(title, fontweight="bold")
        if not strain == 14 and contour ==True:
            ax.contour(cropped_Te < Te_contour, levels=[0.99], extent=ext_Te,
                       colors='k', origin='upper')
        ax.set_autoscale_on(False)      # keeps the crop when faults are overlaid

    Te_contour = (0.5*(Te_twoval_sml + Te_twoval_big)/1e3 if strain == 'twoval'
                  else Te_input/1e3)

    im1 = ax1.imshow(sub_DSP, extent=ext_DSP, origin='upper', cmap=cmap1,
                     vmin=cmap_limits_strain[0], vmax=cmap_limits_strain[1])
    _decorate(ax1, f'DSP - Constant $T_e$={Te_input/1e3:.0f} km', contour=False)

    im2 = ax2.imshow(sub_M5, extent=ext_M5, origin='upper', cmap=cmap1,
                     vmin=cmap_limits_strain[0], vmax=cmap_limits_strain[1])
    if strain == 'twoval':
        ttl2 = f'M5 - 2-Step variable $T_e$=[{Te_twoval_sml/1e3:.0f}, {Te_twoval_big/1e3:.0f}] km'
    elif strain == 14:
        ttl2 = (f'M5 - Plesa $T_e$ map strain=$10^{{-14}}$, '
                f'$T_e\\approx${np.min(T_e_parent_km.data):.1f} km')
    elif strain == 17:
        ttl2 = (f'M5 - Plesa $T_e$ map strain=$10^{{-17}}$, '
                f'$T_e\\approx${np.min(T_e_parent_km.data):.1f} km')
    else:
        ttl2 = f'M5 - Constant $T_e$={Te_input/1e3:.0f} km'
    _decorate(ax2, ttl2, ylabel=False)

    im3 = ax3.imshow(sub_res, extent=ext_DSP, origin='upper', cmap=cmap2,
                     vmin=cmap_limits_strain_diff[0], vmax=cmap_limits_strain_diff[1])
    _decorate(ax3, 'Sum principal strain residual DSP - M5', ylabel=False)

    norm_strain = mcolors.Normalize(vmin=cmap_limits_strain[0], vmax=cmap_limits_strain[1])
    cb1 = fig.colorbar(cm.ScalarMappable(norm=norm_strain, cmap=cmap1),
                       cax=cax_strain_shared, orientation='horizontal')
    cb1.set_label('Sum strain $\\epsilon$ ($\\times 10^{-3}$)', fontweight="bold")

    norm_strain_diff = mcolors.Normalize(vmin=cmap_limits_strain_diff[0],
                                         vmax=cmap_limits_strain_diff[1])
    cb2 = fig.colorbar(cm.ScalarMappable(norm=norm_strain_diff, cmap=cmap2),
                       cax=cax_strain_diff, orientation='horizontal')
    cb2.set_label('Sum strain $\\epsilon$ ($\\times 10^{-3}$)', fontweight="bold")

    # Add extensional tectonic features from Knapmeyer et al. (2006)
    tecto_path = f"{os.getcwd()}/Tectonics_data"
    Plt_tecto_Mars(tecto_path, ax=[ax1, ax2, ax3], compression=False, extension=True)
    # Plt_tecto_Mars(tecto_path, ax=[ax1, ax2, ax3], compression=True, extension=False)

    fig.suptitle(f'Sum horizontal strain Tharsis - contourline at {Te_input/1e3:.1f} km')

    if SaveFigs:
        plt_savetitle = (f'TharsisStrainComp_lmax={LMAX_REF}_iter{iterate}_nmax={nmax}_delta_max={delta_max}_'
                + f'SolveFor{solve_for}_'
                + (f'Mb{Mb/1e3}_Mt{Mt/1e3}_' if solve_for == 'drho_lm' else '')
                + (f'both_constant_Te={Te_input/1e3}km_' if strain == 0 else '')
                + (f'Te_M5=PlesaStrain14Map_Te_DSP={Te_input/1e3}km_' if strain == 14 else '')
                + (f'Te_M5=PlesaStrain17Map_Te_DSP={Te_input/1e3}km_' if strain == 17 else '')
                + (f'Te_M5={Te_twoval_big/1e3:.0f}-{Te_twoval_sml/1e3:.0f}twovalmap_'
                   f'Te_DSP={Te_input/1e3}km_' if strain == 'twoval' else '')
                + f'Tc={T_c/1e3}km'
                + f'filter_on{filter_on}'
                + '.png')
...
# %% TRANSFER FUNCTION IMPULSE TEST
    ...
    (DCfilter_mohoD, DCfilter_mohoDc,
     DCfilter_drhom, DCfilter_drhomc) = DCfilters(lmax_t)

    def ref_w(l, H, G):
        Lap = -l*(l+1.); Lp = Lap + 2.
        Cb  = 3./(rhobar_t*(2*l+1.)); Rl3 = R/(l+3.)
        # DOWNWARD-CONTINUATION FILTER, transcribed straight from
        # thinshell.py 1086-1087 and 1105-1111: eq (1)'s interface term is
        # divided by F1 and eq (2)'s by F2. Only the pair belonging to the
        # active branch is non-unity -- DCfilters handles that.
        DCf_mohoD,  DCf_mohoDc  = DCfilter_mohoD[l],  DCfilter_mohoDc[l]
        DCf_drhom,  DCf_drhomc  = DCfilter_drhom[l],  DCfilter_drhomc[l]
        A = np.zeros((5,5)); rhs = np.zeros(5)
        iw, iGc, iq, iom, iX = 0, 1, 2, 3, 4
        # eq (1) G
        A[0,iw] = Cb*(drhol + drho*phi_t**(l+2)/DCf_mohoD)
        rhs[0]  = G - Cb*rho_l*H
        # eq (2) Gc
        A[1,iGc] = -1.
        A[1,iw]  = Cb*(g0/g_m_t)*(drhol*phi_t**l + drho*phi_t/DCf_mohoDc)
        ...
        if solve_for == 'dc_lm':
            A[0,iX] = -Cb*drho*phi_t**(l+2)/DCf_mohoD
            A[1,iX] = -Cb*(g0/g_m_t)*drho*phi_t/DCf_mohoDc
            A[2,iX] = -g_m_t*drho
            ...
            A[0,iX] = Cb*Rl3*RtbRl3/DCf_drhom
            A[1,iX] = Cb*(g0/g_m_t)*Rl3*(RtRCl - RbRCl)/DCf_drhomc
            A[2,iX] = gdrho_t*M_t_
            A[4,iX] = Phat_t/R
