...
lmax_Te_fit = 90    # LSQ fit resolution (top ~5 degrees never trusted)
...
def load_inputs(lmax, lmax_Te_fit, l_cut, taper_width):
    ...
    T_e_parent, _ = load_Temap(lmax_Te_fit)
...
def derive_D_a(T_e_parent, lmax):
    ...
    T_e_grid_hires = T_e_parent.expand(lmax= 2*lmax_Te_fit)
...
ax.legend()
ax.set_title(f'Power spectra of w lmax comparisons (TSA-B, Plesa Te map, Te loaded at lmax_Te={lmax_Te_fit})')
plt.tight_layout()
...
ax2.set_ylabel(r'$|S_l / S_l^{\rm ref} - 1|$, [%]')
ax2.legend(); ax2.grid(True)
ax2.set_title(f'Residual between lmax={lmax_run} cut at l={lmax_run-5} and lmax_ref={LMAX_REF}')
plt.tight_layout()
plt.savefig(f'{save_path}/'
...
# %%
# T_e_parent120, _     = load_Temap(lmax_Te_fit=120)

# fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 8))
# T_e_parent120.plot_spectrum(show=False, ax=ax1, legend='Te up to l=120')
# T_e_parent70.plot_spectrum(show=False, ax=ax1, legend='Te up to l=70')
# T_e_parent60.plot_spectrum(show=False, ax=ax1, legend='Te up to l=60')
# ax1.set_xlim(0,60)

# l_common = 60
# T_e_diff7060 = truncate(T_e_parent70,   l_common) - truncate(T_e_parent60, l_common)
# T_e_diff12070 = truncate(T_e_parent120,   l_common) - truncate(T_e_parent60, l_common)
# T_e_diff7060.plot_spectrum(show=False, ax=ax2, legend='difference between l=70 and l=60')
# T_e_diff12070.plot_spectrum(show=False, ax=ax2, legend='difference between l=120 and l=60')
# ax2.set_xlim(0, 70)
# plt.show()
